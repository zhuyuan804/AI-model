...
st.set_page_config(
    page_title="Liver Cancer Diagnostic Model",
    page_icon="🧬",
    layout="wide",
...
st.title("🧬 An Interpretable Liver Cancer Diagnostic Model")
st.markdown("""
    This tool uses transcriptome data to predict liver cancer diagnosis and provides interpretable insights 
...
def load_background_data():
    df = pd.read_excel('data/train_10.xlsx')
    return df[['MTRNR2L8', 'PF4', 'TMSB4X', 'HBB', 'PPBP', 'MTRNR2L12', 'FTL', 'OST4', 'S100A9', 'ACTB']]

# Load the pre-trained model
@st.cache_resource
def load_model():
    return tf.keras.models.load_model('data/MODEL_2024_12_31_13_17_17.h5')
...
default_values = {
    'MTRNR2L8': 91.28, 'PF4': 2565.99, 'TMSB4X': 4489.99, 'HBB': 3709.47,
    'PPBP': 1904.12, 'MTRNR2L12': 84.52, 'FTL': 4834.00, 'OST4': 778.97,
    'S100A9': 856.18, 'ACTB': 10072.26
}
...
if st.button("🔍 Calculate SHAP Values", key="calculate"):
    ...
    st.metric("Diagnosis Probability", f"{prediction:.4f}", 
...
with st.expander("ℹ️ About This Model and SHAP Visualizations", expanded=True):
    st.markdown("""
    ### Model Overview
    This model uses 10 key genes from transcriptome data to predict liver cancer diagnosis probability.
